...
void MonsterAi::moveOrAttack(Object *owner, int targetx, int targety) {
  ...
  if( distance < 2.0 ) {
    ...
    double goalMax = -1.0;
    for(int z = 0; z < 8; z++) {
      double goal0 = std::max(engine.map->getGoal(owner->x + dx[z], owner->y + dy[z], 0), 1.0e-3);
      double goal1 = std::max(engine.map->getGoal(owner->x + dx[z], owner->y + dy[z], 1), 1.0e-3);
      double goal = std::max(goal0, goal1);
      if( goal > goalMax ) {
        goalMax = goal;
        iz = z;
      }
