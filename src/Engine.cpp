...
Engine::Engine(int windowWidth, int windowHeight) : gameStatus(STARTUP),fovRadius(12),
  ...
  mapWidth(2*windowWidth),mapHeight(2*(windowHeight - 9)), level(0) {
...
bool Engine::update() {
  ...
  switch( gameStatus ) {
    ...
    case DEFEAT: {
      ...
      con.printFrame(0, 0, DIALOG_WIDTH, DIALOG_HEIGHT, true, TCOD_BKGND_DEFAULT, "Defeat");
      ...
      if( key.vk == TCODK_ENTER ) status = false;
